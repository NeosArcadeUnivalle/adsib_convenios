...
export default function ConvenioDetalle() {
  ...
  // total de versiones YA existentes en el convenio (todas las páginas)
  const totalVersionesActuales =
    typeof meta.total === "number" && meta.total >= 0
      ? meta.total
      : versiones.length;

  // Regla de negocio:
  // Solo se puede cerrar si el convenio YA tiene al menos 2 versiones previas.
  // Si solo existe v1 (total = 1), no se puede marcar FINAL.
  const puedeCerrarConEsta = totalVersionesActuales >= 2 && !isClosed;
  ...
  const crearVersion = async (e) => {
    ...
    if (finalizar) {
      // total de versiones que ya existen ANTES de guardar la nueva
      const totalExistentes = totalVersionesActuales;

      // ❗ Regla: si solo hay v1 (total = 1), NO se puede cerrar.
      if (totalExistentes < 2) {
        alert(
          "No puedes cerrar la negociación de un convenio que solo tiene la versión inicial.\n\n" +
            "Primero registra al menos una versión adicional (por ejemplo, v2) y luego podrás marcar una versión como FINAL."
        );
        return;
      }
  ...
  const descargarV = async (vid) => {
    try {
      const res = await api.get(`/versiones/${vid}/descargar`, {
        responseType: "blob",
      });
    ...
    <div className="card" style={{ padding: 20 }}>
      ...
      >
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <Link to="/" className="btn" style={BTN.back}>
            Volver
          </Link>
      ...
      <div className="card" style={{ marginTop: 14 }}>
        ...
        {isClosed && (
          ...
          >
            <div style={{ fontSize: 14 }}>
              <strong>Convenio cerrado.</strong> No es posible subir nuevas
              versiones.
            </div>
          ...
          >
            <label
              style={{
                ...
                cursor:
                  formDisabled || !puedeCerrarConEsta
                    ? "not-allowed"
                    : "pointer",
                opacity:
                  formDisabled || !puedeCerrarConEsta
              ...
              title={
                puedeCerrarConEsta
                  ? "Marca esta casilla si esta versión es la final. Se cerrará la negociación."
                  : "Para marcar una versión como FINAL el convenio debe tener al menos 2 versiones ya registradas."
              }
            >
              <input
                type="checkbox"
                checked={finalizar && puedeCerrarConEsta}
                onChange={(e) => setFinalizar(e.target.checked)}
                style={{ width: 18, height: 18 }}
                disabled={formDisabled || !puedeCerrarConEsta}
              />
              <div style={{ display: "grid" }}>
                ...
                <span style={{ fontSize: 12, opacity: 0.9 }}>
                  Cierra la negociación del convenio
                </span>
              </div>
            </label>
        ...
        >
          ...
          <span style={{ opacity: 0.9 }}>
            Página {meta.current_page || page} / {meta.last_page || 1}
          </span>
          <button
            className="btn"
            style={{
              ...BTN.dark,
              ...(page >= (meta.last_page || 1) ? BTN.disabled : {}),
            }}
