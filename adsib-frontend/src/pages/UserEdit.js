...
export default function UserEdit(){
  ...
  useEffect(()=>{
    ...
    api.get(`/usuarios/${id}`).then(r=>{
      if(!alive) return;
      setF(s=>({ ...s, nombre:r.data.nombre, email:r.data.email }));
    });
    return ()=>{ alive=false; };
  },[id]);
  ...
  const submit = async(e)=>{
    ...
    try{
      const payload = { nombre:f.nombre, email:f.email };
      ...
      <form className="card" onSubmit={submit}>
        ...
        <div className="grid" style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:12}}>
          ...
          <label>Confirmar nueva contraseña
            <input className="input" type="password" value={f.password_confirmation} onChange={e=>setF({...f, password_confirmation:e.target.value})} />
          </label>
        </div>
